...
function renderBelongings(){const q=norm($('belongings-search').value),cat=$('belongings-category').value,loc=$('belongings-location').value;let items=array(state.belongings).filter(i=>(!q||norm([i.name,i.location,i.detail].join(' ')).includes(q))&&(!cat||i.category===cat)&&(!loc||i.location===loc));items=sortBelongings(items);$('belongings-list').innerHTML=items.length?items.map(i=>{const linked=i.inventoryItemId?state.shopping.inventory.find(inv=>inv.id===i.inventoryItemId):null,stockHtml=linked?`<span class="pill">現在在庫 ${linked.confirmedQuantity==null?'要確認':esc(linked.confirmedQuantity)}${esc(linked.unit||'')}</span>`:'';return`<article class="list-card"><h3>${esc(i.name||'名称未設定')}</h3><div class="meta"><span class="pill">📍 ${esc(i.location||'場所未設定')}</span>${i.category?`<span>${esc(i.category)}</span>`:''}${stockHtml}${i.detail?`<span>${esc(i.detail)}</span>`:''}</div><div class="item-actions"><button data-action="edit-belonging" data-id="${esc(i.id)}">編集・移動</button><button class="danger-small" data-action="delete-belonging" data-id="${esc(i.id)}">削除</button></div></article>`}).join(''):'<div class="empty">該当する持ち物はありません</div>'}
...
// 未入力のまま1として確定させることはしない（0より大きい実数の明示入力を必須にする）。
let purchaseQtyShoppingId=null,purchaseQtyMode=null,purchaseQtyConfirmed=null;
function openPurchaseQtyModal(shoppingId,mode){const item=state.shopping.shoppingList.find(i=>i.id===shoppingId);if(!item)return;purchaseQtyShoppingId=shoppingId;purchaseQtyMode=mode;$('purchase-qty-name').textContent=item.productName||'名称未設定';$('purchase-qty-planned').textContent=item.plannedQuantity!=null?`${item.plannedQuantity}${item.unit||''}`:'未設定';$('purchase-qty-unit-label').textContent=item.unit||'';$('purchase-qty-input').value=item.plannedQuantity??'';$('purchase-qty-modal').hidden=false;setTimeout(()=>$('purchase-qty-input').focus(),30)}
...
// ---- ホーム在庫チェックから「買うものに追加」 ----
// plannedQuantityを推測しない：入力欄は常に空欄で始まり、0より大きい数値の
// 明示入力を必須にする（「通常購入数量」の正式データがshoppingList/inventory
// どちらにも存在しないため）。単位のみ、既存の在庫データから初期値を補う。
let homeAddShoppingInventoryId=null;
function openHomeAddShoppingModal(id){const item=state.shopping.inventory.find(i=>i.id===id);if(!item)return;homeAddShoppingInventoryId=id;$('home-add-shopping-name').textContent=item.productName||'名称未設定';$('home-add-shopping-qty').value='';$('home-add-shopping-unit').value=item.unit||'';$('home-add-shopping-modal').hidden=false;setTimeout(()=>$('home-add-shopping-qty').focus(),30)}
function closeHomeAddShoppingModal(){$('home-add-shopping-modal').hidden=true;homeAddShoppingInventoryId=null}
function confirmHomeAddShopping(){
  const item=state.shopping.inventory.find(i=>i.id===homeAddShoppingInventoryId);
  if(!item)return;
  const raw=$('home-add-shopping-qty').value,qty=Number(raw);
